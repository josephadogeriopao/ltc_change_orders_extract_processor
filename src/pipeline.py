...
# import sys
# import logging
# from constants.constants import PropertyType, JOB_REGISTRY
...
#
#     try:
#         # Resolve absolute template path relative to where job_config.py is safely stored
...
"""
Core Ingestion Engine Pipeline Organizer
----------------------------------------
Description: Orchestrates file path allocations and feeds verified tracker sheets
             and templates down into the processing automation layers.
...
# 🔧 PATH FIX: Updated imports to match your new modular directory layout
from constants.job_config import PropertyType, JOB_REGISTRY
from utils.mailmerge import process_change_orders
...
def run_pipeline(excel_file_path: str, property_type: PropertyType, base_output_dir: str, ui_app=None) -> None:
    ...
    try:
        # 🔑 PATH FIX: Grab the template path directly from our fixed runtime registry.
        # This already contains our robust sys._MEIPASS and source-directory checks.
        raw_template_path = JOB_REGISTRY[property_type].template_path

        print("path to doc file, ", raw_template_path)

        # 2. Trigger framework processing engine exactly once
        # 🔧 PATH FIX: Changed template_doc to read raw_template_path directly.
        # REMOVED: os.path.abspath(raw_template_path) which was breaking relative path detection!
        word_app = process_change_orders(
            excel_file=os.path.abspath(excel_file_path),
            template_doc=raw_template_path,
            output_dir=output_dir,
            master_docx_path=master_docx_path,
            master_pdf_path=master_pdf_path,
            test_sample_pdf_path=test_sample_pdf_path,
            ui_app=ui_app
        ) # ✅ CODE FIX: Closed the parenthesis safely here to resolve syntax breaks!
